...
项目目前处于 M4 阶段：文稿规划、素材搜索、本地 Review Server 和 Project API 已完成，React Review Board UI 计划在 M5 实现。
...
M4 实现了一个本地 HTTP Review Server，供未来 React Review Board（M5）和 CLI 用户在浏览器或工具中审核场景、选择候选、上传本地素材。
...
M1～M4 已完成。本地 Review Server 和 Project API 可用。后续按 M5～M6 实现 React Review Board UI 和发布。
