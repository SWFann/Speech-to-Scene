...
项目目前处于 M5 阶段：文稿规划、素材搜索、本地 Review Server、Project API 和 React Review Board UI 已完成，`s2s review` 可直接服务 React 构建产物。
...
M1～M5 已完成。本地 Review Server、Project API 和 React Review Board UI 可用。`s2s review` 可直接服务 React 构建产物。后续按 M6 进行最终审计与发布。
